...
arrend:
.text
la t0  array # в t0 хранится адресс начала массива
...
fill: #заполняем массив числами 
	bge t3 t1 end_fill # в t3 будет храниться счетчик для цикла
	li a7 5
	ecall
	mv t2, a0
...
ev_odd: # цикл, в котором считаем кол-во четных и нечетных чисел, в регистрах s2 и s3
	...
	lw t5 (t0)
	rem t6 t5 t2 # остаток числа массива при делении на 2
	even:
		bnez t6, odd
		addi s2, s2, 1
	odd:
...
sum:
	bge t3 t1 end_sum
	lw t5 (t0)  # распаковка текущего числа массива
	li s5 0 # тут хранятся результаты xor
	li s4 0 # тут хранится знак новой суммы, после прибавления нового числа
	li s6 0# тут хранится знак текущей суммы, перед добавлением нового числа
	li s7 0 # тут хранится знак текущего элемента массива
